...
class Line:
    ...
    def process_message(self, message):
        ...
        value = message.value()
        topic = value.topic()
        if 'cta.traffic.stations.transformed' == topic: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif 'cta.traffic.arrival.' in topic: # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif 'TURNSTILE_SUMMARY' in topic: # Set the conditional to the KSQL Turnstile Summary Topic
